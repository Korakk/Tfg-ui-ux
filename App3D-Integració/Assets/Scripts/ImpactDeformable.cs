...
{  
    ...
    public bool DeformMeshCollider = false;
    ...
    // Deformation force event (deformable, point, force)
    public event Action<ImpactDeformable, Vector3, Vector3> OnDeformForce;

    // Deformation event (deformable)
    ...
    // Last calculated structural damage
    float structuralDamage = 0;
    ...
    {
        newpos = col.gameObject.transform.position;
    ...
    void ProcessCollision(Collision col)
    {
        ...
        for (int c = 0; c < col.contacts.Length; c++) {
            ...
            if (col.gameObject.CompareTag("hands")) {
                Vector3 velocity = new Vector3((oldpos.x - newpos.x) * 2, (oldpos.y - newpos.y) * 2, (oldpos.z - newpos.z) * 2);
                (col.contacts[c].thisCollider.GetComponent<ImpactDeformable>() ?? this)
                // Process collision point
                //.ProcessContactPoint(col.contacts[c].point, new Vector3(col.relativeVelocity.x, -3, col.relativeVelocity.z), col.contacts[c].normal);
                .ProcessContactPoint(col.contacts[c].point, velocity, col.contacts[c].normal);
            }
            else
                (col.contacts[c].thisCollider.GetComponent<ImpactDeformable>() ?? this)
                // Process collision point
    ...
    public void Deform(Vector3 point, Vector3 force)
    {
        ...
        // Check if any vertex was deformed
        if (deformed)
        {
            if (Application.isEditor)
                ...
                ApplyChangesToMesh();
            if (OnDeformForce != null)
                OnDeformForce(this, point, force);
        }
    }

    // Start the update of the mesh with deformed data, keep one deformation per frame if not requested immediate mode
    void ApplyChangesToMesh()
    {
        if (applyingChanges)
            return;
    ...
    // Apply deformation data to mesh on next end of frame
    IEnumerator DeferredApplyChanges()
    {
        // Wait next frame to apply collision deformation
    ...
    {
        // Reset structural damage to recalculate it on next access
        structuralDamage = -1;
        ...
        {
            // Return to undeformed mesh as collider

            meshCollider.sharedMesh = null;

            Mesh colliderMesh = new Mesh();
            colliderMesh.vertices = meshCache.Vertices;
            colliderMesh.triangles = meshCache.Triangles;
    ...
    // Estimate structural damage (or use cached value)
    // Result is the average vertex deformation of entire mesh
    public float StructuralDamage
    {
        get
        {
            // Check if we can use the previous cached value
            if (structuralDamage == -1)
            {
                // Check if there is a mesh to deform
                ...
                for (int i = 0; i < deformedVertices.Length; i++)
                {
                    ...
                    Vector3 vc = meshCache.Vertices[i];
                    deformation += new Vector3(Mathf.Abs(v.x - vc.x), Mathf.Abs(v.y - vc.y), Mathf.Abs(v.z - vc.z));
                }

                deformation /= deformedVertices.Length;
                deformation.Scale(meshCache.SizeFactor);
                structuralDamage = deformation.magnitude;
            }
    ...
    // Average structural damage for this deformable and sub colliders
    public float AverageStructuralDamage
    {
        get
        {
            return transform
                .GetComponentsInChildren<ImpactDeformable>()
                .Where(i => i.MeshFilter != null)
                .Select(i => i.StructuralDamage)
                .DefaultIfEmpty(0)
                .Average();
        }
    }
