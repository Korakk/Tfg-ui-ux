...
public class Meshinator : MonoBehaviour
{
    ...
    private Vector3 oldPosLeft, newPosLeft, oldPosRight, newPosRight, velocityLeft, velocityRight;
	...
	public void OnCollisionEnter(Collision collision)
	{
		...
		//if (m_ClearedForCollisions && collision.relativeVelocity.magnitude >= m_ForceResistance)
		//{
			...
			foreach (ContactPoint contact in collision.contacts)
			{
				...
				{
                    Debug.Log("Velocity: " + velocityRight);
                    //Impact(contact.point, collision.relativeVelocity, m_ImpactShape, m_ImpactType);
                    if (collision.gameObject.CompareTag("handLeft"))
                        Impact(contact.point, velocityLeft, m_ImpactShape, m_ImpactType);
                    else if (collision.gameObject.CompareTag("handRight"))
                        Impact(contact.point, velocityRight, m_ImpactShape, m_ImpactType);
                    else
                        Impact(contact.point, collision.relativeVelocity, m_ImpactShape, m_ImpactType);
                    break;
				}
    ...
    public void updatePositions()
    {
        ...
        velocityLeft = new Vector3((oldPosLeft.x - newPosLeft.x) * 2, (oldPosLeft.y - newPosLeft.y) * 2, (oldPosLeft.z - newPosLeft.z) * 2);
        velocityRight = new Vector3((oldPosRight.x - newPosRight.x) * 2, (oldPosRight.y - newPosRight.y) * 2, (oldPosRight.z - newPosRight.z) * 2);
    }
